...
- For every pair of workout modifiers and every workout muscle bucket, require at least five approved exercises under each of the four real UI states: on/on, on/off, off/on, and off/off. A restrictive modifier off relaxes its predicate; the Mirror equipment modifier off excludes only `MirrorOnly` exercises. In every Mirror-on pair state, the five-exercise floor must be supplied by `MirrorOnly` plus `BenefitsGreatly`; `Agnostic` remains selectable but cannot satisfy that floor. Separately require restrictive modifiers to exclude, and Mirror categories 1–2 jointly to supply, a material anatomically broad set both alone and when either member of a pair is already enabled. Keep this validation quadratic in the number of modifiers; do not replace it with an exponential all-modifier quota or an independent per-modifier approximation.
