...
- For every pair of workout modifiers and every workout muscle bucket, require at least five approved exercises under each of the four real UI states: on/on, on/off, off/on, and off/off. A restrictive modifier off relaxes its predicate; the Mirror equipment modifier off excludes only `MirrorOnly` exercises. Whenever Mirror is on, only categories 1–2 (`MirrorOnly` and `BenefitsGreatly`) may satisfy its five-exercise floor; `Agnostic` exercises remain selectable but do not satisfy Mirror coverage. Never inflate a relationship category to satisfy this floor: genuine deficits must remain visible until filled by valid exercises. Separately require restrictive modifiers to exclude, and Mirror categories 1–2 jointly to supply, a material anatomically broad set both alone and when either member of a pair is already enabled. Keep this validation quadratic in the number of modifiers; do not replace it with an exponential all-modifier quota or an independent per-modifier approximation.
...
- Keep Light outside pairwise, per-muscle, duration, materiality, and catalog-coverage quotas. It is a session-scoped ranking control, not an exercise-compatibility predicate: within the highest persisted preference bucket it favors only atomic sequences whose every distinct member has `muscularDemand = 0`, without promoting a lower-score exercise, deleting a Keep, or filtering nonzero work when no such top-bucket sequence exists. Derive the automatic day-4/8/12 default from completed local-calendar training days, but always allow the user to override it before or during a workout and never persist Light as the next session's physical setup.
