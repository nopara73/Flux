...
test("hard floor catalog verdicts include slippery-floor traction", () => {
  assert.equal(catalog.filter((exercise) =>
      ...
      EXERCISE_HARD_FLOOR_COMPATIBILITY.Compatible).length, 351);
  assert.equal(catalog.filter((exercise) =>
    exercise.hardFloorCompatibility ===
...
test("reviewed production catalog satisfies the enforceable coverage hierarchy", () => {
  ...
  assert.equal(catalog.filter((exercise) =>
    exercise.upperBodyClothingRequirement ===
      EXERCISE_UPPER_BODY_CLOTHING_REQUIREMENT.Agnostic).length, 529);
  assert.equal(catalog.filter((exercise) =>
    exercise.shyCompatibility === EXERCISE_SHY_COMPATIBILITY.Compatible).length, 426);
  assert.equal(catalog.filter((exercise) =>
  ...
  assert.equal(catalog.filter((exercise) =>
    exercise.mirrorRelationship === EXERCISE_MIRROR_RELATIONSHIP.Agnostic).length, 444);
  assert.equal(catalog.filter((exercise) =>
    exercise.mirrorRelationship ===
...
test("the reviewed catalog satisfies every roll-up and selects distinct exercises", () => {
  assert.equal(catalog.length, 543);
  assert.equal(new Set(catalog.map((exercise) => exercise.id)).size, 543);
  assert.equal(new Set(catalog.map((exercise) => exercise.name)).size, 543);
...
test("sole-wall revision rebuilds changed workout state and resets scores", () => {
  ...
  assert.equal(CURRENT_CATALOG_REVISION, 77);
  assert.deepEqual(
