...
- “Tap to keep” is the only rest decision.
- If tapped during the rest window: score is unchanged and that slot retains the exercise.
- If not tapped by rest resolution: decrement that exercise's integer score exactly once and mark only that slot for replacement.
